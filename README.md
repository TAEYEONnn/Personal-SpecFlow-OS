...
- OpenAI 및 NVIDIA Build API 기반 AI 문서 생성 (환경변수로 전환)
...
- OpenAI Responses API / NVIDIA Build API
- React Flow와 Dagre
...
- `SUPABASE_SERVICE_ROLE_KEY`
- `NVIDIA_API_KEY` — `NEXT_PUBLIC_` 접두사 절대 금지
- `OPENAI_API_KEY`
- `NOTION_CLIENT_SECRET`
...
`AI_PROVIDER` 환경변수로 OpenAI와 NVIDIA Build API를 전환할 수 있습니다.

### NVIDIA Build (기본값)

1. [NVIDIA Build](https://build.nvidia.com/)에서 API 키를 발급합니다.
2. `.env.local`에 `AI_PROVIDER=nvidia`와 `NVIDIA_API_KEY`를 설정합니다.
3. `NVIDIA_STRUCTURED_MODE=prompt`가 기본값입니다. `guided-json`은 NVIDIA Hosted API 호환성 확인 후 사용하세요.
4. 무료 Endpoint는 프로토타입 검증 전용입니다. 운영 전 품질·지연시간·사용 한도를 별도로 검증하세요.

### OpenAI (폴백)

`AI_PROVIDER=openai`로 즉시 OpenAI로 롤백할 수 있습니다.
...
```bash
pnpm ai:smoke:nvidia
```

API 키가 없으면 안전하게 중단됩니다. CI에서는 실행하지 않습니다.

### 개발 데모 모드

Supabase 환경변수가 없고 AI 키도 없는 개발 환경에서는 데모 모드가 활성화됩니다. API 키가 있어도 `mode: "demo"`로 명시하면 외부 API를 호출하지 않습니다.
